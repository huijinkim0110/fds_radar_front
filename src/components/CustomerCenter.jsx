...
export default function CustomerCenter() {
  ...
  const filteredFaqs = FAQS.filter((item) => {
    ...
    const questionInitials = getInitials(question);
    const answerInitials = getInitials(answer);

    return (
      question.includes(query) ||
      answer.includes(query) ||
      questionInitials.includes(query) ||
      answerInitials.includes(query)
    );
  });
      ...
      >
        ...
        >
          ...
          <div style={{ padding: "8px 0" }}>
            <input
              type="text"
              placeholder="궁금한 내용을 검색해보세요 (예: 송금, ㅅㄱ, FDS, ㅂㅁㅂㅎ)"
              value={searchQuery}
              onChange={(e) => {
